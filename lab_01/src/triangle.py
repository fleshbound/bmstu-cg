...
class Triangle(object):
    ...
    def __init__(self, point_a, point_b, point_c):
        ...
        self.height_point = Point(0, 0)
        self.bisector_point = Point(0, 0)

    def set_min_angle_unique_data(self):
        abc_height_p = get_height_point(self.point_b, self.point_a, self.point_c)
        cab_height_p = get_height_point(self.point_a, self.point_b, self.point_c)
        bca_height_p = get_height_point(self.point_c, self.point_b, self.point_a)
        ...
        abc_unique_angle = get_angle_between_lines_by_points(self.point_b, abc_height_p, abc_bisect_p)
        cab_unique_angle = get_angle_between_lines_by_points(self.point_b, cab_height_p, cab_bisect_p)
        bca_unique_angle = get_angle_between_lines_by_points(self.point_b, bca_height_p, bca_bisect_p)
        ...
        if abc_unique_angle <= cab_unique_angle:
            if abc_unique_angle <= bca_unique_angle:
                ...
                self.set_height_point(abc_height_p)
        ...
        if cab_unique_angle <= abc_unique_angle:
            if cab_unique_angle <= bca_unique_angle:
                ...
                self.set_height_point(cab_height_p)
        ...
        if bca_unique_angle <= cab_unique_angle:
            if bca_unique_angle <= abc_unique_angle:
                ...
                self.set_height_point(bca_height_p)

    def get_bisector_line(self):
        return get_line_by_points(self.unique_angle_point, self.bisector_point)

    def get_height_line(self):
        return get_line_by_points(self.unique_angle_point, self.height_point)

    def set_height_point(self, point):
        self.height_point.set_x(point.x)
        self.height_point.set_y(point.y)

    def set_bisect_point(self, point):
        self.bisector_point.set_x(point.x)
        self.bisector_point.set_y(point.y)

    def get_bisector_point(self):
        return self.bisector_point

    def get_height_point(self):
        return self.height_point
